...
# cria uma tibble no formato amplo (desorganizada)
receitas <- tribble(
  ~produto, ~T1, ~T2, ~T3, ~T4,
...
# Transforma receitas para o formato longo(organiza os dados)
...
# cria uma tibble no formato amplo (desorganizado)
desempenho <- tribble(
...
# Transforma desempenho para o formato longo (organizado)

desempenho_longo <- desempenho |> 
  pivot_longer(
    cols = -empresa,
    names_to = c("indicador", "trimestre"), #(C cria um vetor)
    names_sep = "_",
    values_to = "valor"
  )
...
# DESAFIO: Combine as 3 tabelas com um único pipeline usando left_join ()
