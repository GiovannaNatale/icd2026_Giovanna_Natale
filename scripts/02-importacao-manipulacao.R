...
options(digits = 5, scipen = 999) #para não exibir números em notação cientifica
...
# Salva os dados limpos em um arquivo rds para aálises futuras sem precisar repetir 
...
## 1. Defina o caminho relativo do arquivo rds
caminho_rds <- here("dados/limpos/dados_vendas_limpos.rds")
...
# Filtra as mesmas vendas usando %in%, uma forma mais compacta para 
# múltiplas comparações da mesma variável
dados_vendas_limpos |> 
...
# Cria a variável prec0_desconto (10% sobre o preco_unitario)
dados_vendas_limpos |> 
...
# Cria a variável receita total, agrupa em cidade, calcula a receita total
# por cidade e ordena o resultado
dados_vendas_limpos |> 
  mutate(receita_total = unidades * preco_unitario) |> 
  group_by(cidade) |> 
  summarise(receita_tota_cidade = sum(receita_total)) |> 
  arrange(desc(receita_tota_cidade))
...
# Cria a variável categoria_receita com multiplas categorias
dados_vendas_limpos |> 
...
# Calcula a receita média por cidade produto
dados_vendas_limpos |> 
